...
Project Codex overlays are limited to `AGENTS.md`, `AGENTS.override.md`, `.codex/config.toml`, `.codex/hooks.json`, `.codex/skills/`, and `.agents/skills/`. Guidance-file symlinks for `AGENTS.md` and `AGENTS.override.md` are dereferenced and materialized as regular files after the same size, hardlink, executable, and UTF-8 checks as regular guidance files. Config and hook files are not copied when they are symlinks or fail those regular-file checks. Skill trees are copied more permissively: symlinked entries may be dereferenced, regular files are materialized as-is, and traversal-like names, `.git`, virtualenvs, `node_modules`, and hidden entries outside `.system/` are skipped. `.codex/rules/`, `.codex/hooks/`, plugin integration state, and hidden skill files such as `.env` or `.credentials.json` are not copied. When an unsafe guidance/config/hook overlay file is skipped, the startup warning includes the skipped source path.
