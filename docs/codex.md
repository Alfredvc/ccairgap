...
Project Codex overlays are limited to `AGENTS.md`, `AGENTS.override.md`, `.codex/config.toml`, `.codex/hooks.json`, `.codex/skills/`, and `.agents/skills/`. Guidance-file symlinks for `AGENTS.md` and `AGENTS.override.md` are dereferenced and materialized as regular files after the same size, hardlink, executable, and UTF-8 checks as regular guidance files. Skill trees may also dereference symlinked entries, while skipping `.git`, virtualenvs, `node_modules`, and hidden entries outside `.system/`. Hardlinks, traversal-like paths, hidden credentials, oversized files, executable active files, config and hook symlinks, `.codex/rules/`, `.codex/hooks/`, plugin integration state, and non-UTF-8 files are omitted. When an unsafe user or project Codex overlay file is skipped, the startup warning includes the skipped source path.
