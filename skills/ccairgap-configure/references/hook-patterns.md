...
All Claude Code hooks are **disabled by default** inside ccairgap. Opt back in with `--hook-enable <glob>` (repeatable) or `hooks.enable: [<glob>, …]` in config.
...
ccairgap hooks --repo ~/src/foo --extra-repo ~/src/bar
```
