...
All Claude Code MCP servers are **disabled by default** inside ccairgap. Opt back in with `--mcp-enable <glob>` (repeatable) or `mcp.enable: [<glob>, …]` in config.
...
- Project-scope servers (`<repo>/.mcp.json`) come from repo authors the user may not fully trust — Claude Code's approval dialog is unreachable inside a non-interactive sandbox.
...
**User-scope** (top-level `~/.claude.json`) and **plugin-scope** (enabled plugins) have no such gate. Glob match alone is sufficient — the user put it in their own config / enabled the plugin themselves.
...
- **Enabling ≠ working.** The glob decides what gets past the sandbox. Starting the server still requires:
  ...
  - Any env vars / credentials passed through (`docker-run-arg: ["-e API_KEY"]` — see `secrets-and-sensitive-data.md`).
  - For HTTP/SSE transports, network reachability from the container.
- **Project-scope servers need two gates.** Pass `--mcp-enable` AND make sure the server is approved on host. A user who says "I want my project's grafana MCP enabled" but hasn't clicked approve on the host will get an empty `mcpServers` inside the container. Tell them to approve on host first.
...
- User says "I need the grafana MCP in the sandbox" → confirm it's declared (`ccairgap inspect`), add `mcp.enable: ["grafana"]`. If it's project-scope and `approvalState: unapproved`, tell them to approve on host first (`/mcp` TUI or add to `enabledMcpjsonServers`).
- User says "I want all my MCPs" → list every distinct name from `ccairgap inspect`. If it's a dozen, `"*"` is legal but loses the filter's safety net. Tell them what they're opting into (list the names and what each needs to actually work — binary + env).
- User says "MCPs are noise, I don't want any" → leave `mcp.enable` unset. Default behavior.
- User's MCP references a binary that isn't in the base image or needs a secret → either extend the Dockerfile + pass the secret via `docker-run-arg: ["-e NAME"]`, or leave it disabled. Don't enable MCPs you know will fail at start.
