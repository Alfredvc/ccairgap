...
`;
  ...
  it("rejects repo that is not array of strings", () => {
    expect(() => parseConfig("repo: /single\n", SRC)).toThrow(
      /config\.repo: expected array of strings/,
    );
    expect(() => parseConfig("repo: [1, 2]\n", SRC)).toThrow(
      /config\.repo: expected string/,
    );
  });
...
describe("resolveConfigPaths", () => {
  it("resolves relative repo/ro/dockerfile against config dir", () => {
    const cfg = {
      repo: ["./sub", "/abs/path"],
      ro: ["../ro"],
      dockerfile: "./Dockerfile",
    ...
    expect(out).toEqual({
      repo: ["/repo/.claude-airgap/sub", "/abs/path"],
      ro: ["/repo/ro"],
